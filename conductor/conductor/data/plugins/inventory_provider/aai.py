...
class AAI(base.InventoryProviderBase):
            ...
            for requirement in requirements:
                ...
                if inventory_type == 'cloud':
                    ...
                    if filtering_attributes:
                        ...
                        resolved_demands[name].extend(SDC.get_sdc_response(sdc_candidates_list))
    ...
    def get_nst_response(self, filtering_attributes):
        raw_path = 'service-design-and-creation/models' + aai_utils.add_query_params_and_depth(filtering_attributes, "2")
        path = self._aai_versioned_path(raw_path)
        aai_response = self._request('get', path, data=None)
            ...
            for nst_metadata in nst_metadatas:
                ...
                for model_ver in model_vers:
                    ...
                    nst_candidate = NST(instance_info=nst_info, model_ver=model_ver_info, info=info,
                                        default_fields=aai_utils.convert_hyphen_to_under_score(default_attributes),profile_info=None)
                    # candidate = nst_candidate.convert_nested_dict_to_dict()
