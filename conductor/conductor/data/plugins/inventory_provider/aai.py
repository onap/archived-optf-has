...
from conductor.data.plugins.inventory_provider import aai_utils
...
from conductor.data.plugins.inventory_provider.candidates.nssi_candidate import NSSI
...
class AAI(base.InventoryProviderBase):
    ...
    def get_nssi_candidates(self, filtering_attributes, candidate_uniqueness):
        ...
        return self.filter_nssi_candidates(aai_response.json(), second_level_match, candidate_uniqueness)
        ...
        if filtering_attributes and response_body is not None:
            ...
            for nssi_instance in nssi_instances:
                inventory_attributes = aai_utils.get_inv_values_for_second_level_filter(filtering_attributes,
                                                                                        nssi_instance)
                nssi_info = self.get_instance_info(nssi_instance)
                if self.match_inventory_attributes(filtering_attributes, inventory_attributes,
                    ...
                    for slice_profile in slice_profiles:
                        slice_profile_id = slice_profile.get('profile-id')
                        info = Candidate.build_candidate_info('aai', 'nssi', self.conf.data.nssi_candidate_cost,
                                                              candidate_uniqueness, slice_profile_id)
                        slice_profile_info = aai_utils.convert_hyphen_to_under_score(slice_profile)
                        nssi_candidate = NSSI(nssi=nssi_info, slice_profile=slice_profile_info, info=info)
                        candidate = nssi_candidate.convert_nested_dict_to_dict()
