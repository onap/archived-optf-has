...
class DCAE(object):
    ...
    def capacity_filter(self, candidates):
        ...
        print("from AAI", candidates) 
        for candidate in candidates:
            print("single candidate", candidate)
            ...
            print("capacity filter ", candidatesList)
            updated_candidateList.append(candidatesList)
            print("updated candidate list", updated_candidateList)
    ...
    def _request(self, method='get', path='/', data=None,
        ...
        LOG.debug("Total time for DCAE request "
                   "({0:}: {1:}): {2:.3f} sec".format(context, value, elapsed))
        if response is None:
            LOG.error(_LE("No response from DCAE ({}: {})").
                    format(context, value))
        elif response.status_code != 200:
            LOG.error(_LE("DCAE request ({}: {}) returned HTTP "
                "status {} {}, link: {}{}").format(context, value,
                    response.status_code, response.reason, self.base, path))
        return response
