...
tenant/3c6c471ada7747fe8ff7f28e100b61e8/vservers/vserver/00bddefc-126e-4e4f-a18d-99b94d8d9a31")
    ...
    def test_get_nssi_candidates(self):
        ...
        self.assertEqual(nssi_candidates, self.aai_ep.filter_nssi_candidates(nssi_response, filtering_attributes))

        nssi_response['service-instance'][0]['orchestration-status'] = 'deactivated'

        self.assertEqual([], self.aai_ep.filter_nssi_candidates(nssi_response, filtering_attributes))

        nssi_response['service-instance'][0]['service-role'] = 'service'

        self.assertEqual([], self.aai_ep.filter_nssi_candidates(nssi_response, filtering_attributes))

        self.assertEqual([], self.aai_ep.filter_nssi_candidates(None, filtering_attributes))

        self.assertEqual([], self.aai_ep.filter_nssi_candidates(None, None))

        self.assertEqual([], self.aai_ep.filter_nssi_candidates(nssi_response, None))
